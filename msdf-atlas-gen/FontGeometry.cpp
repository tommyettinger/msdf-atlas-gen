...
bool FontGeometry::loadMetrics(msdfgen::FontHandle *font, double fontScale) {
    if (!msdfgen::getFontMetrics(metrics, font, msdfgen::FontCoordinateScaling::KEEP_INTEGERS))
        return false;
    if (metrics.emSize <= 0)
...
int FontGeometry::loadKerning(msdfgen::FontHandle *font) {
    ...
    for (size_t i = rangeStart; i < rangeEnd; ++i)
        for (size_t j = rangeStart; j < rangeEnd; ++j) {
            double advance;
            if (msdfgen::getKerning(advance, font, (*glyphs)[i].getGlyphIndex(), (*glyphs)[j].getGlyphIndex(), msdfgen::FontCoordinateScaling::KEEP_INTEGERS) && advance) {
